...
resource "aws_secretsmanager_secret" "bootstrap_token" {
  #count = var.acls ? 1 : 0
  name                    = "${random_string.randomprefix.result}${var.name}-bootstrap-token"
...
resource "aws_secretsmanager_secret" "gossip_key" {
  ...
  name                    = "consul_server_${random_string.randomprefix.result}${var.suffix}-gossip-encryption-key"
  recovery_window_in_days = 0
}
...
resource "aws_secretsmanager_secret" "ca_key" {
  #count = var.tls ? 1 : 0
  name = "${random_string.randomprefix.result}${var.name}-ca-key"
}
...
resource "aws_secretsmanager_secret" "ca_cert" {
  #count = var.tls ? 1 : 0
  name = "${random_string.randomprefix.result}${var.name}-ca-cert"
}
